...
def get_full_tracks(
    ...
    stems: Sequence[str] = ("non_speech","speech"),
):
...
def evaluate_separations(
    ...
    for track, separated_track in get_full_tracks(separation_path, separation_sr, stems):
        ...
        print(f"Dimensions for track: {track}")
        original_track = load_and_resample_track(dataset_path/track, stems, 22050)

        for stem, audio in original_track.items():
            print(f"  Original {stem}: {audio.shape}")
        ...
        for stem, audio in separated_track.items():
            print(f"  Separated {stem}: {audio.shape}")
