...
def load_audio(file, sr, offset, duration, resample=True, approx=False, time_base="samples", check_duration=True):
    ...
    for frame in container.decode(audio=0):  # Only first audio stream
        ...
        # Check if frame is a list
        if isinstance(frame, list):
            frame = [f.to_ndarray(format="fltp") for f in frame]
            frame = np.concatenate(frame, axis=-1)  # Combine all frames
        else:
    ...
    # print(f"Loaded audio shape: {sig.shape}, sr: {sr}")
    return sig, sr
...
class MultiSourceDataset(Dataset):
    ...
    def filter(self, tracks):
        ...
        for track in tracks:
            ...
            files = []
            for root, dirs, filenames in os.walk(track_dir):
                for filename in filenames:
                    if filename.lower().endswith(('.mp3', '.opus', '.m4a', '.aac', '.wav')):
    ...
    def init_dataset(self):
        ...
        print(f"Found {len(tracks)} tracks in {self.audio_files_dir}.")
        if not tracks:
            raise FileNotFoundError(f"No tracks found in directory: {self.audio_files_dir}")
        self.filter(tracks)
        print(f"Dataset initialized with {len(self.tracks)} tracks.")
        tracks = os.listdir(self.audio_files_dir)
    ...
    def get_song_chunk(self, index, offset):
        ...
        for stem in self.stems:
            ...
            # print(f"Loaded {stem} data shape: {data.shape}")
    ...
    def get_item(self, item):
        ...
        transformed_wav = self.transform(torch.from_numpy(wav))
        # print(f"Final output shape: {transformed_wav.shape}")
